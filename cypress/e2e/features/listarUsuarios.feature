...
@tag12Usuarios 
Cenário: Testando os botões de páginação com 12 usuários cadastrados no banco de dados
    E vi que havia 12 usuários cadastrados
    Quando clico no botão de próximo
    Então o site passa para a página 2 
    Quando clico no botão de anterior
    Então ele volta para a página 1
